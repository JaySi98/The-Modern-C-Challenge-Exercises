...
uint32_t FindHighestCommonDivisior(uint32_t firstNumber, uint32_t secondNumber);
...
int main(int argc, char **argv)
{
    if(IsArgumentsCorrect(argc,argv))
    {
        ...
        uint32_t result = FindHighestCommonDivisior(firstNumber, secondNumber);
...
uint32_t FindHighestCommonDivisior(uint32_t first, uint32_t second)
{
    while(second != 0)
    {
